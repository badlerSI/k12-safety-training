...
- **I trusted vLLM's `load_lora_adapter` success response without verifying base ≠ LoRA at inference.** For several hours I was reporting blindspot trajectory numbers that were entirely noise on base model output — vLLM accepted the LoRA registration but silently skipped applying BF16 LoRA weights against the FP8 quantized base I was serving. The trajectory ranges I confidently reported (V13-B "5→7→6→8") were sampling noise on the same underlying base model. The fix is a one-line precondition check (call the model with a known-trained prompt against base and LoRA — they MUST differ); the diagnostic methodology contribution is that any multi-LoRA eval pipeline needs this check baked in. Documented in [`eval_design.md`](eval_design.md). I should have built this on day one.

- **I trusted the naive substring `must_not_appear` scorer instead of mirroring production's `_output_looks_like_refusal` auto-tune.** Hand-triage of 28 failures showed the scorer overcounted failures by ~2.7× because clean refusals naming the pattern they're refusing match the same forbidden-phrase substring. Production ShieldGemma already handles this correctly via threshold auto-tuning; the blindspot scorer should mirror that logic, not reinvent a less-careful version. Methodology choice documented in [`eval_design.md`](eval_design.md); per-case triage in [`safety_findings.md`](safety_findings.md) Finding #8.

- **I designed the V13-A `chosen` examples to be 'pedagogical pivots' away from the harmful ask, without sufficient validators that the pivot didn't carry too much of the original request.** Hand-triage of V13-A regressions reveals that the model has learned a "refuse the persona + deliver the persona's content in a slightly-modified shape" pattern that almost exactly mirrors the `refuse_then_comply` pattern V13 was designed to suppress. The training data taught the failure pattern instead of suppressing it — because the `chosen` field carried a watered-down version of the harmful content with a refusal prefix, the model generalized that shape to deliver the actual harmful content with a refusal prefix. **`chosen` validators need to be strict enough that "refusal + watered-down version" is also rejected, not just "compliance".** Detailed in [`safety_findings.md`](safety_findings.md) Finding #8.
