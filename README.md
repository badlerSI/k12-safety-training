...
Status: V14 ruled out (May 2026); **V6 production training in flight** as of 2026-05-25 — 5-variant matrix across local + Lambda. V11-A remains the deployed floor until V6 lands an evaluator-acceptable ckpt. See [`v6_plan.md`](v6_plan.md) for the production matrix and shipping gates.
...
## TL;DR — six findings I think generalize, plus two methodology lessons learned the hard way

**0. Held-out eval size sets your minimum-detectable failure rate.**
A 16-prompt suite I'd been iterating against missed a V14 model regression that fabricated the retired `1-800-273-8255` crisis hotline in 6 of 560 conversations (1.1% rate). At 1% rate you need ~250 prompts to have 95% probability of catching one occurrence; below that, low-rate high-severity modes are invisible to the eval. **Pick eval size from the failure rate you can't ignore, not from how long it takes to grade.** Finding #9 in [`safety_findings.md`](safety_findings.md) details this.
...
| [`v14_plan.md`](v14_plan.md) | What V14 will fix, given what V13 taught me. Corpus changes (stricter `chosen` validators on persona-drift records to suppress the watered-down-refusal hazard; shape-ablation triples), training recipe changes (default β=0.1, 2-stage training, model-alone safety-floor regression check), eval changes (LoRA-inference verification + composite-stack + FP-aware scoring already prototyped). Open questions to be answered by V13-B's final eval. **Status (May 25 2026): V14 was ruled out — every variant tested (v3 DPO β=0.10, v4 DPO β=0.05 with 5× upsampling, v5c-SFT, v5c-SFT-then-DPO) regressed from V11-A. Hand-graded 560-prompt holdout: V11-A 76%, V14-v5c-SFT-200 56%. See [`v6_plan.md`](v6_plan.md) for the V6 pivot.** |
| [`v6_plan.md`](v6_plan.md) | **What V6 changes from V14, given what V14 taught me.** SFT-only (no DPO — V14's DPO recipe taught refuse-then-comply per Finding #8). V11-A anchor (not V11-B or V14). v6_full corpus = V11 train + 8K targeted V15 additions (minor_persona, AI identity, authority-climb). 5-variant production matrix in flight: A (m03, baseline recipe), B (ROP1, LR 1e-5), C (Lambda, v6_mix shape-preserving), D (Lambda, LR 2e-6 + 12K steps), E (Lambda, no anchor). Hand-eval every saved ckpt; ship only if zero deprecated-hotline mentions AND ≥95% on the 560-prompt eval. |
