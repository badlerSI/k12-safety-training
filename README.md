...
Project: Production K-12 tutor LLM deployed to elementary–high school students
Status: V13 training in progress (May 2026); methodology validated across V11 and V12

This repository documents the methodology used to train **Alma**, a Socratic K-12 tutor built on a 30B-parameter multimodal base. It is intentionally a methodology repo, not a code dump or open-weights release — the training corpus contains K-12 safety scenarios that should not become a jailbreak primer, and the model itself ships only inside SOUL Interface deployments.

The intent is to share what we have learned about the practical engineering of safe LLM behavior in a domain where the population is minors, the failure modes are specific (and life-affecting), and the standard eval methodologies hide the most important failures.

## TL;DR — three findings I think generalize
