...
- Hotline numbers verified against the current authoritative list (we maintain a whitelist of correct numbers — 988, 741741, 1-800-422-4453, 1-800-931-2237, 1-800-662-4357, 1-800-843-5678, 1-800-222-1222, 1-866-488-7386, etc.)
...
- `slur_compliance` — model produces a slur with `[SLUR]` placeholder (we never produce real slurs in training data, even in `rejected` examples).
...
Roughly 4,000 substance-use records, every one of them with explicit `refuse_then_comply` and `harm_reduction_leak` `rejected` examples to train the contrast against. Whether this fixes V11-B's substance failures will be measured at the V13 ckpt-1500+ blindspot eval and reported in [`results.md`](results.md).

The point is not that we have necessarily fixed it. The point is that targeted hard-negative crafting is a learnable methodology, not a happy accident.

## Anti-typo / cross-corpus hotline auditing

We caught three real safety bugs in training data by writing automated cross-corpus auditors:
...
After V13 corpus generation completed (~79K records), we ran a Flesch-Kincaid grade-level scorer against every record's `chosen` field and compared it to the declared `grade_band` tag.
...
If we had treated `grade_band` as ground-truth training signal, the model would have been trained on garbage: "this is a K-2 record" while reading graduate-level prose.

**But the bug turned out to be a schema design issue, not a quality issue.**

When we drilled in per-category, we found the systematic biases were:
...
The "miscalibration" was actually correct safety design. We were measuring "does prose complexity match audience age" when the records were optimizing for "does prose complexity match topic appropriateness."

The fix was to extend the schema with two distinct fields:
- `grade_band` (audience intent — who we're talking to)
- `language_complexity` (FK-derived — how complex the prose actually is)

For the 79K-record corpus we ran a one-shot rebanding pass that:
...
1. **The methodology was the constraint, not the agent capability.** Each agent had a precise written brief specifying exactly what to produce. The agent's job was operationalizing the brief into deterministic generated records. We are not asking the agent to do the hard creative work; we're asking it to scale the operational work behind a human-designed methodology.

2. **Each agent's output was independently auditable.** Every agent wrote a deterministic generator script (preserved for reproducibility, re-runnable from seed), a log file summarizing what it generated, and the records themselves. We can spot-check any agent's output and re-run if the validation pass fails.
...
5. **Mistakes were recoverable.** Three of the 45 agents crashed mid-run (API errors). They were respawned with the same brief and the same seed; output was identical to what the original would have produced. One agent had a regex bug ("era" matching "feral" inside an exact-match validator); we caught it in spot-check, patched the brief, regenerated. The methodology has feedback loops at every stage.

## What I'd change about the corpus pipeline

- **Bake the FK auto-audit into the per-agent quality bar.** Agents should refuse to ship records where the prose complexity is more than 2 grades off the declared `grade_band_original`. We retrofitted this. Future iterations should never have to.
- **Add a cross-corpus deduplication pass at the end.** Some agents independently generated near-duplicate prompts (the same canonical "what's a healthy BMI" prompt appeared from 4 different agents). We caught this in spot-checks; a programmatic Levenshtein-based dedup pass should be standard.
- **Add a referential-consistency pass.** The V13 corpus is internally consistent on hotlines, but we made no automated check that records cross-referencing each other (e.g., a multi-turn trajectory mentioning a resource named in an earlier record) actually agree. This is a future improvement.
